    ...
    <div class="animated fadeIn">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('structure-division.update', $data->id) }}" method="post" class="form-vertical" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="form-group mb-3">
                                <label for="hf-nama" class="form-control-label">Nama Pengurus</label>
                                <input type="text" id="hf-nama" name="nama" value="{{ $data->name }}" class="form-control @error('nama') is-invalid @enderror" >
                                @error('nama')
                            ...
                            <input type="hidden" name="old_picture" value="{{ $data->images }}">
                            <div class="form-group mb-3">
                                <label for="hf-posisi" class="form-control-label">Posisi Pengurus</label>
                                <input type="text" id="hf-posisi" name="posisi" value="{{ $data->position }}" class="form-control @error('posisi') is-invalid @enderror" >
                                @error('posisi')
                                <div class="invalid-feedback">
                                    <small class="help-block form-text text-danger">{{$message}}</small>
                                </div>
                            ...
                            <img src="{{ asset('img/pengurus/'.$data->images) }}" height="300" class="mb-3">
                            <div class="form-group mb-3">
                                <label for="hf-foto" class="form-control-label">Foto Pengurus</label>
                                <input type="file" id="hf-foto" name="foto" class="form-control @error('foto') is-invalid @enderror">
                                @error('foto')
                                <div class="invalid-feedback">
                                    <small class="help-block form-text text-danger">{{$message}}</small>
                                </div>
